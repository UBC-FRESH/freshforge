...
def test_release_version_contract_is_0_1_0a3() -> None:
    pyproject = tomllib.loads((ROOT / "pyproject.toml").read_text(encoding="utf-8"))

    assert pyproject["project"]["version"] == "0.1.0a3"
    assert freshforge.__version__ == "0.1.0a3"
...
def test_release_notes_document_alpha_boundaries() -> None:
    ...
    assert "0.1.0a3" in release_notes
    assert "not published to PyPI" in release_notes
    assert "run namespaces" in release_notes
